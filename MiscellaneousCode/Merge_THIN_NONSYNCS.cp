...
for (LINK pL = clipboard->headL; pL!=clipboard->tailL; pL = RightLINK(pL))
LogPrintf(LOG_DEBUG, "MergeSetupClip: pL=%u spare=%d\n", pL, LinkSPAREFLAG(pL));
						...
						)
{
	...
	/* The temporary object list now looks like this:
	   ...
	   If <thin>, remove everything in it that came from the clipboard (which _should_
	   be absolutely everything in it, but be cautious) other than Syncs. */

LogPrintf(LOG_DEBUG, "MRearrangeAll: thin=%d firstL=%u tailL=%d\n", thin, firstL, tailL);
Browser(doc, firstL, tailL);
	if (thin) ThinNonSyncObjs(doc, firstL, tailL);	// ?????TESTING!!!!!!!!!!!
