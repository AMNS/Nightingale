...
#ifdef NOMORE
/* ------------------------------------------------------------ ReconstructArray -- */
/* Loop through the entire durArray: create syncs out of notes which have the
same pTimes, and insert them between prevL and insertL. */
		
Boolean ReconstructArray(Document *doc, PTIME *durArray, INT16 arrBound, LINK prevL,
									LINK insertL)
{
	INT16 i,subCount,objSel,v;
	PTIME *qTime,*pTime;
	LINK newObjL,firstSubObj,subL,newSubL,tempSubL;
	PMEVENT pObj,pNewObj;

	qTime=pTime=durArray;
	for (i=0; i<arrBound; pTime=qTime) {
		...
		if (subCount) {
			newObjL = NewNode(doc, SYNCtype, subCount);
			if (newObjL == NILINK) return FALSE;
			...
			/* Copy the subObject information from each note at this time into
				...
				This refers to the sync still in the object list. */
