...
static void UseMultivoiceRules(Document *doc, LINK barFirstL, LINK barLastL,
											INT16 staff, CONTEXT context)
{
	...
	for (s = 0; s<=doc->nstaves; s++)
		voiceFound[s] = NOONE;					/* No voices found yet ??IGNORE STAVES OTHER THAN staff! */
	...
	for (pL = barFirstL; pL!=barLastL; pL = RightLINK(pL))
		if (ObjLType(pL)==SYNCtype) {
			...
			for ( ; aNoteL; aNoteL=NextNOTEL(aNoteL)) {
				aNote = GetPANOTE(aNoteL);
				if (voiceFound[aNote->staffn]==NOONE)
					voiceFound[aNote->staffn] = aNote->voice;		/* Remember 1st voice */

				if (voiceFound[aNote->staffn]==aNote->voice) {
					if (MainNote(aNoteL)) {							/* Skip stemless notes in chords */
						tempystem = CalcYStem(doc, aNote->yd, NFLAGS(aNote->subType),
													FALSE,				/* Stem up */
													context.staffHeight, context.staffLines,
													config.stemLen2v, FALSE);
						aNote = GetPANOTE(aNoteL);
						aNote->ystem = tempystem;
					}
				}
...
/* ---------------------------------------------------------- TryMultivoiceRules -- */
/* Check every measure on every staff from startL to endL; if a given
measure/staff contains any notes/rests for more than one voice, apply "multi-
voice notation rules": point stems in opposite directions, move slurs and ties
to the outside, etc. etc. */

void TryMultivoiceRules(Document *doc, LINK startL, LINK endL)
{
	LINK		startBarL, endBarL,
				barFirstL, barLastL;
	Boolean	done, multivoice[MAXSTAVES+1];
	...
	startBarL = LSSearch(LeftLINK(startL), MEASUREtype, 1, TRUE, FALSE); /* Start at previous Measure */
	if (!startBarL)																		/* If none... */
		startBarL = LSSearch(doc->headL, MEASUREtype, 1, FALSE, FALSE);	/* ...start at first Measure */
	...
	{
		barLastL = EndMeasSearch(doc, barFirstL);
		if (!BeforeFirstMeas(barFirstL))
		{
			FindMultivoice(doc, barFirstL, barLastL, multivoice);
			for (s = 0; s<=doc->nstaves; s++)
				if (multivoice[s])
					UseMultivoiceRules(doc, barFirstL, barLastL, s, context[s]);
		}
		if (barLastL==endBarL) done = TRUE;
		barFirstL = barLastL;									/* Skip to next MEASURE obj */		
	}
