...
void GetNString(LINK graphicL, char theStr[])
{
	...
	Pstrcpy((StringPtr)theStr, (StringPtr)PCopy(theStrOffset));
...
INT16 HasSectionIDPrefix(char theStr[256])
{
	...
	if (str1len>0 &&
		strncmp(theStr, SECTION_ID_STR1, str1len)==0)
			return str1len;
	if (str2len>0 &&
		strncmp(theStr, SECTION_ID_STR2, str2len)==0)
			return str2len;
...
//DebugPrintf(">SearchGetHitSet(startL=%d): foundL=%d\n", startL, foundL);
	if (foundL==DB_NILINK)
		return DB_NILINK;							/* Scanned the rest of the score and didn't find the pattern */
	
	/* We found matches; return the first one in an acceptable voice, if any. Note
	 * that to avoid disasterous interactions with Search Again, we need the same
	 * concept of voice order it has.
	 */
	matchVoice = 0;
	needStartVoice = TRUE;
	...
	if (matchVoice==0) {
		...
		foundL = SearchGetHitSet(doc, RightLINK(startL), searchPat, sParm, foundVoiceA, totalErrorInfo,
										matchedObjA, matchedSubobjA);
...
find an instance, select its notes/rests, display it, and return TRUE; if we don't
find one or there's an error, return FALSE.
...
{
	DB_LINK foundL, startL; INT16 foundVoice;
	Boolean haveChord;
	INT16 startVoice;
	...
	if (CapsLockKeyDown()) {
		...
		DebugPrintf("%s", str);
		DebugPrintf(", L%d V%d: ", startL, startVoice);
		if (foundL==DB_NILINK)
...
/* Search the given score for all instances of the given Search Pattern. If we
find any instances, add them to a list starting at a given location, and return
the new total number of hits found. */
					...
					INT16 nFound)									/* Number of hits found so far */
{
	...
	while (TRUE) {
		...
		/* Found instance(s) of the pattern: save them. Then prepare to look for
		 * another instance, starting with the Sync after the one these instances
		 * started with.
		 */
...
/* Set *fs to the next file or folder in an iteration over all the contents of
the previously-declared scan folder. If *isFolder is FALSE, then the file's
creator, type, modDate, and vRefNum are delivered in the appropriate variables.
Delivers TRUE if found another file/folder; FALSE if scan is complete, or if any
error occurred.  If file or folder, its last modification date is also returned.
creator, type, or modDate can be NULL if you're not interested in them. */

Boolean FSpGetNextFile(FSSpec *fs, Boolean *isFolder, OSType *creator, OSType 
*type, unsigned long *modDate, short *ioVRefNum, short *ioFRefNum, StringPtr
ioNamePtr)
{
	CInfoPBRec info; OSErr err;

	/* First get the usual paramblock file information out of the system for
	 * the next index'ed file.
	 */
	info.hFileInfo.ioCompletion = NULL;
	info.hFileInfo.ioNamePtr = fs->name;
	...
	if (err == noErr) {
		...
		if (info.dirInfo.ioFlAttrib & ioDirMask) {
			*isFolder = TRUE;
			...
			if (ioNamePtr)
				PStrCopy(info.hFileInfo.ioNamePtr, ioNamePtr);
		}
		 else {
			*isFolder = FALSE;
			if (creator)
...
/* Convert a folder's file spec to its directory ID (which is different from
the FSSpec's parID field). Sets dirID the answer, or delivers paramErr if file
spec wasn't a folder; or other system error. If folder is NULL, delivers the
current scan dirID. */
...
static Document *SSFSpecOpenDocument(FSSpec *theFile)
{
	...
	switch (fndrInfo.fdType) {
		case DOCUMENT_TYPE_NORMAL:
			if (DoOpenDocument(theFile->name, wdRec.ioVRefNum, OPEN_READ_ONLY,theFile)) break;
			return NULL;
		default:
...
static INT16 GetDirFileInfo(FSSpec *pFSFolder, FSSpec docFSSpecA[], short maxDocs)
{
	...
	do {
		...
		if (okay) {
		/* We have a file or folder. If it's a file containing an Ngale score, save its info. */
			if (!isFolder && type==documentType) {
				...
				if (CapsLockKeyDown() && OptionKeyDown())
					DebugPrintf("  creator=%T type=%T modDate=%lu name=%s\n", creator, type, modDate, itemName);
...
Boolean AnyOpenDocWindows()
{
	 ...
	 */
	for (wp = (WindowPeek)LMGetWindowList(); wp; wp = wp->nextWindow) {
		kind = wp->windowKind;
		
#ifndef PUBLIC_VERSION
		if (CapsLockKeyDown() && OptionKeyDown()) {
			DebugPrintf("AnyOpenDocWindows: wp=%lx kind=%d ", wp, kind);
			DebugPrintf("%c ", (wp==(WindowPeek)clipboard? 'C' : '-'));
			DebugPrintf("%p\n", (kind==DOCUMENTKIND? ((Document *)wp)->name : "\p "));
		}
#endif
		if (kind == DOCUMENTKIND) {
#ifdef TEST_SEARCH_NG
			if (wp!=(WindowPeek)clipboard && wp!=(WindowPeek)searchPatDoc) return TRUE;
#else
			if (wp!=(WindowPeek)clipboard) return TRUE;
#endif
...
Boolean AnyOpenDocWindows()
{
	...
	for (wp = FrontWindow(); wp!=NULL; wp = GetNextWindow(wp)) {		
		...
		if (CapsLockKeyDown() && OptionKeyDown()) {
			DebugPrintf("AnyOpenDocWindows: wp=%lx kind=%d ", wp, kind);
			DebugPrintf("%c ", (wp==(WindowPtr)clipboard? 'C' : '-'));
			DebugPrintf("%p\n", (kind==DOCUMENTKIND? ((Document *)wp)->name : "\p "));
		}
#endif
		if (kind == DOCUMENTKIND) {
...
#else
			if (wp!=(WindowPtr)clipboard) return TRUE;
#endif
							...
							Boolean pitchKeepContour, INT16 chordNotes, Boolean matchTiedDur)
{
	...
	DebugPrintf("DoIRSearchFiles: FSSpec=%lx\n", pTheFile);
	...
	for (ind = 0; ind<totalDocs; ind++) {
		PStrCopy(docFSSpecA[ind].name, itemName);
		PToCString(itemName);
		if (CapsLockKeyDown()) DebugPrintf("ind=%d name=%s\n", ind, itemName);
		...
		/* If we didn't find any matches in the score, close it. ??If doc was open before the search, should probably leave it open regardless! */
...
Cleanup:
	...
	if (infoIOWarning.nDocsMissingFonts>0 ||
		...
		infoIOWarning.nDocsPageSetupPblm>0 ||
		infoIOWarning.nDocsOther>0)
			DebugPrintf("Warning: nDocsMissingFonts=%d nDocsNonstandardSize=%d nDocsTooManyPages=%d nDocsPageSetupPblm=%d nDocsOther=%d\n",
