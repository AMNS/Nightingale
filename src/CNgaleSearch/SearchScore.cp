...
INT16 DB_TiedLDur(DB_LINK, DB_LINK);
INT16 DB_TiedLDur(DB_LINK syncL, DB_LINK aNoteL)
{
					...
					)
{
	 ...
	 */
	if (DB_NoteREST(theNoteL)!=searchPat.noteRest[targetPos])
		return FALSE;
	if (DB_NoteTIEDL(theNoteL)!=searchPat.tiedL[targetPos])
		return FALSE;
	...
	if (usePitch) {
		 ...
		 * in the pattern and decide if there's a match. If "note" is a rest, the difference
		 * is always zero and there's always a match.
		 */
		...
		switch (pitchSearchType) {
			...
			case TYPE_PITCHMIDI_REL:
				 ...
				 * other note, check the interval from the previous note (not rest!) against
...
targetPos, intervalWanted, intervalHere);
				...
				/* If we're going up instead of down, or down instead of staying the same, etc.,
				 * and <pitchKeepContour>, then fail regardless of <pitchTolerance>.
				 */
...
static long nPRTotal, nPRSoFar, nPRUpdatesSoFar, updatePRSkip;

Boolean ShowingProgressReport(void);
void InitProgressReport(long nPRTotal);
...
INT16 SyncMatch(SEARCHPARAMS sParm, SEARCHPAT searchPat, INT16 voice, DB_LINK syncL, DB_LINK
						...
						ERRINFO *pErrInfo);
INT16 RestartSearchTargetPos(SEARCHPARAMS sParm, SEARCHPAT searchPat, DB_LINK failedMatchStartL,
...
void FormatTime(long timeMilliSecs, char *str);
void FormatTime(
...
static void SortMatches(MATCHINFO matchInfoA[], INT16 nsize);
static void SortMatches(MATCHINFO matchInfoA[], INT16 nsize)
{
	INT16 i, j; MATCHINFO temp;
					...
					)
{
	...
	for (n = 0; n<nFound; n++) {
		...
		DebugPrintf("%s\n", str);
#ifndef SEARCH_DBFORMAT_MEF
		// ??FOR MEF, CASTING foundL AS LINK WITH A LARGE MEF FILE WILL FAIL!
