...
static void DimPitchPanel(DialogPtr dlog, short pitchItem)
{
	...
	if (matchPitch) {
		...
		if (relPitch) {
			...
			PenMode(patBic);
			/*
			 * GetDialogItem should get the bounding box of the panel we want to dim;
...
/* Maintain state of items subordinate to the Match Duration checkbox. If
	...
	while (keepGoing) {
		...
		switch (itemHit) {
			...
			case BUT34_FindAll:
				...
				/* The displayed state of "always preserve contour" may be affected
				 * by other options, so pick up the saved state instead.
				 */
...
the dlog opened, or NULL if error (no resource, no memory); return in parameters
the radio-button group settings. */
