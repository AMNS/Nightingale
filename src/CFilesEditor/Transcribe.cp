...
static Boolean InitQuantize(Document *doc, Boolean merge)
{
	...
	/* For now, no time sigs. are allowed in the range to be quantized, so this is easy:
	   the table always has just one entry. */
...
rawSyncTab[] and rawNoteAux[], merge the results into <doc>, and add rests between notes.
InitQuantize _must_ be called before this! Returns FAILURE if there's a problem, else
					...
					)
{
	...
	/* We have one or more notes. Convert and merge them. <maxNewSyncs> has to be large
	   enough for all existing Syncs plus any generated by clarifying. I can't think of
	   a good way to guess how much clarifying to allow for. But <maxNewSyncs> certainly
	   can't be more than 65535, since that's the most notes/rests we can have in an
	   entire score! For now, make a wild but generous guess. */
	...
	/* At this point, Measure <qExtraMeasL> of the score contains ALL of the quantized
	   notes/chords, [newSyncTab[0], newSyncTab[nRawSyncs-1]], plus possible ties and
	   tuplets; we want to merge all of it starting at the next Measure. Note that the
	   number of objects in that Measure can greatly exceed MAX_MEASNODES, so a lot of
	   Nightingale's machinery can't be used now. */
	...
	/* Merge everything into its proper place. NB: MRMerge gets timing information from
	   the LINKTIMEINFO arrays, not from the object list! */
...
/* --------------------------------------------------------------------- GDNewMeasure -- */
/* Add a new Measure object just before <pL> and leave an insertion point set just before
<pL>. At the moment, virtually identical to MFNewMeasure. */
...
to the contents of the Measures. Finally, offset the timestamps of all Measures after that
accordingly (preserving their durations). NB: MIDIFileOpen.c contains a similar function:
...
/* --------------------------------------------------------------------- GDAddMeasures -- */
/* Just before <qEndL>, add an "extra" Measure, which should later be removed, to create
a temporary storage area, and add Measures as described by <measInfoTab[0]> (at some
future time we may support time sig. changes, as Import MIDI File's MFAddMeasures does).
Set the new Measures' timestamps to give them the durations indicated by the time
signature, with the extra Measure's timestamp set to 0. Return the first new Measure,
the "extra" Measure. If we fail (due to lack of memory), return NILINK. NB: Destroys the
selection range and leaves timestamps in an inconsistent state! */
...
static void DelAndMoveMeasure(Document *doc, LINK measL)
{
	...
	/* Adjust objects' x-coords. to make relative to their new Measure but keep them in
...
/* Quantize the selection. Restrictions:
	...
	- If !merge, doc->selStartL must be the first obj in a Measure, and selEndL must end
	  a Measure.
The quantized notes will have redundant accidentals; call DelRedundantAccs to remove them.
		...
		)
{
	...
	/* Clean up. Kill the extra Measure (see comments in Voice2KnownDurs); try to get
	   rid of unisons in chords; etc. We need to handle unisons because the process we've
	   gone thru ignores the notes' current spellings, so it can turn minor seconds into
	   augmented unisons. */
...
/* Get the nos. of the Measures enclosing the selection range. If <selEndL> is after the
last Measure, return -1 for <*pEndNum>. */
				...
				)
{
	...
	/* Check if any Measure in the score has too much in it; if so, for now, give up. It
	   would be much better to just add Measures as needed to keep under the limit, but
	   this should very rarely happen anyway. */
...
/* ------------------------------------------------------------------- QuantizeSelDurs -- */
/* Extend the selection to include everything in every measure in the selection range;
then quantize the attack time and duration of every selected note/rest/chord to a grid
whose points are separated by the duration described by <quantumDur>. If there are
pauses between the notes, intersperse rests as necessary. Return True if successful. NB:
leaves timestamps in an inconsistent state: the calling function should call
...
//LogPrintf(LOG_DEBUG, "QuantFilter: box tlbr=%d,%d,%d,%d\n", box.top, box.left, box.bottom,
//box.right);
...
static Boolean QuantizeDialog(Document *doc,
								short *durCode,
								Boolean *tuplet,
...
LogPrintf(LOG_DEBUG, "QuantizeDialog: lDurCode=%d choiceIdx=%d\n", lDurCode, choiceIdx);
	...
	while (!done) {
		...
		switch (ditem) {
		case OK:
			/* Unless secret key is down, don't allow triplets if the quantum is
			   shorter than a sixteenth. This was to sidestep a bug that caused triplets
			   containing unknown-duration notes, but I doubt whether this was enough to
			   avoid the problem completely, and I think the bug has been fixed anyway
			   (as of 1.0b14). --DAB */
				
			tups = GetDlgChkRadio(dlog, QTUPLET_DI);
			if (!(ControlKeyDown()) && tups && lDurCode>SIXTEENTH_L_DUR) {
...
LogPrintf(LOG_DEBUG, "TranscribeDialog: oldChoiceIdx=%d choiceIdx=%d byChLeftPos=%d\n",
...
{
	static short lDurCode = EIGHTH_L_DUR;
	static Boolean tuplet = False, autoBeam = False;

	if (QuantizeDialog(doc, &lDurCode, &tuplet, &autoBeam)) {
		PrepareUndo(doc, doc->selStartL, U_Quantize, 37);		/* "Undo Transcribe Recording" */
		if (QuantizeSelDurs(doc, lDurCode, tuplet)) {
...
{	LINK pL;
	...
	if (!QuantizeDialog(doc, &lDurCode, &tuplet, &autoBeam)) return False;

	if (!RTMQuantizeSelDurs(doc, lDurCode, tuplet, staffn)) return False;

	/* Start fixing timestamps before the range quantized so the Measure starting the
	   range has the correct timestamp. */
